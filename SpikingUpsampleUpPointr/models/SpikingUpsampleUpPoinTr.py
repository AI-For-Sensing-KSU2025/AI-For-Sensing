...
import spikingjelly.activation_based import neuron, layer, functional
...
class SpikingConv1dBlock(nn.Module):
    ...
    Expects input shape: (T, B, C_in, N)
    Returns:            (T, B, C_out, N)
    """
    def __init__(self, in_channels, out_channels, kernel_size=1):
        super().__init__()
        # SpikingJelly expects (T, B, C, L)
        self.conv = layer.Conv1d(in_channels, out_channels, kernel_size, bias=True)
        # LIF neuron
        self.neuron = neuron.LIFNode(tau=2.0, surrogate_function=neuron.SurrogateFunctionBase())

    def forward(self, x_seq):
        # x_seq: (T, B, C_in, N)
        x_seq = self.conv(x_seq)
        x_seq = self.neuron(x_seq)
        return x_seq


class SpikingUpTransformer(nn.Module):
    def __init__(self, in_channel, out_channel, dim, n_knn=20, up_factor=2,
                 use_upfeat=True, pos_hidden_dim=64, attn_hidden_multiplier=4,
                 scale_layer=nn.Softmax, attn_channel=True, T=4):
        super().__init__()
        self.n_knn = n_knn
        ...
        # Spiking version of mlp_v
        self.mlp_v_spike = SpikingConv1dBlock(in_channel * 2, in_channel)

        # Spiking projections for key, query, value, upfeat
        self.conv_key_spike   = SpikingConv1dBlock(in_channel, dim)
        self.conv_query_spike = SpikingConv1dBlock(in_channel, dim)
        self.conv_value_spike = SpikingConv1dBlock(in_channel, dim)
        if use_upfeat:
            self.conv_upfeat_spike = SpikingConv1dBlock(in_channel, dim)
        else:
            self.conv_upfeat_spike = None

        self.pos_conv1 = nn.Conv2d(3, pos_hidden_dim, 1)
        self.pos_act1  = neuron.LIFNode(tau=2.0, surrogate_function=neuron.SurrogateFunctionBase())
        self.pos_conv2 = nn.Conv2d(pos_hidden_dim, dim, 1)
        self.pos_act2  = neuron.LIFNode(tau=2.0, surrogate_function=neuron.SurrogateFunctionBase())

        # Attention
        attn_hidden = dim * attn_hidden_multiplier
        self.attn_conv1 = nn.Conv2d(dim, attn_hidden, 1)
        self.attn_act1  = neuron.LIFNode(tau=2.0, surrogate_function=neuron.SurrogateFunctionBase())
        if up_factor:
            self.attn_conv2 = nn.ConvTranspose2d(attn_hidden, attn_out_channel,
                                                 kernel_size=(up_factor, 1),
                                                 stride=(up_factor, 1))
        else:
            self.attn_conv2 = nn.Conv2d(attn_hidden, attn_out_channel, 1)

        self.scale = scale_layer(dim=-1) if scale_layer is not None else nn.Identity()
        # Upsample value and identity
        ...
        # Spiking conv for final output channels
        self.conv_end_spike = SpikingConv1dBlock(dim, out_channel)

        # Residual path (non spiking)
    ...
    def forward(self, pos, key, query, upfeat):
        ...
        Inputs:
            ...
            upfeat:(B, in_channel, N)
        """
        B, _, N = key.shape

        # Shape: (T, B, C, N)
        key_seq   = key.unsqueeze(0).repeat(self.T, 1, 1, 1)
        query_seq = query.unsqueeze(0).repeat(self.T, 1, 1, 1)
        up_seq    = upfeat.unsqueeze(0).repeat(self.T, 1, 1, 1)

        # Compute spiking value
        v_in_seq  = torch.cat([key_seq, query_seq], dim=2)
        v_seq     = self.mlp_v_spike(v_in_seq)
        value_seq = v_seq
        identity  = value_seq[-1]

        # Project to key/query/value spaces
        key_proj_seq   = self.conv_key_spike(key_seq)          # (T, B, dim, N)
        query_proj_seq = self.conv_query_spike(query_seq)      # (T, B, dim, N)
        value_proj_seq = self.conv_value_spike(value_seq)      # (T, B, dim, N)

        # Take last timestep
        key_proj   = key_proj_seq[-1]      # (B, dim, N)
        query_proj = query_proj_seq[-1]    # (B, dim, N)
        value_proj = value_proj_seq[-1]    # (B, dim, N)

        # KNN (non-spiking)
        pos_flipped = pos.permute(0, 2, 1).contiguous()        # (B, N, 3)
        idx_knn = query_knn(self.n_knn, pos_flipped, pos_flipped)  # (B, N, k)

        # Group neighbors shape= (B, dim, N, k)
        key_group   = grouping_operation(key_proj, idx_knn)
        pos_group   = grouping_operation(pos,      idx_knn)
        value_group = grouping_operation(value_proj, idx_knn)

        # Relative features and positional embedding (spiking)
        q_reshaped = query_proj.view(B, -1, N, 1)              # (B, dim, N, 1)
        qk_rel     = q_reshaped - key_group                    # (B, dim, N, k)

        pos_reshaped = pos.view(B, -1, N, 1)                   # (B, 3, N, 1)
        pos_rel      = pos_reshaped - pos_group                # (B, 3, N, k)

        # Positional embedding with LIF
        pos_emb = self.pos_conv1(pos_rel)                      # (B, pos_hidden_dim, N, k)
        pos_emb = self.pos_act1(pos_emb)
        pos_emb = self.pos_conv2(pos_emb)                      # (B, dim, N, k)
        pos_emb = self.pos_act2(pos_emb)

        # Upfeat embedding spiking
        if self.use_upfeat:
            up_proj_seq = self.conv_upfeat_spike(up_seq)       # (T, B, dim, N)
            up_proj     = up_proj_seq[-1]                      # (B, dim, N)
            up_group    = grouping_operation(up_proj, idx_knn) # (B, dim, N, k)
            up_rel      = up_proj.view(B, -1, N, 1) - up_group # (B, dim, N, k)
        else:
            up_rel = torch.zeros_like(qk_rel)

        # Spiking attention
        attn_input = qk_rel + pos_emb + up_rel                 # (B, dim, N, k)
        h = self.attn_conv1(attn_input)                        # (B, dim*mult, N, k)
        h = self.attn_act1(h)

        h = self.attn_conv2(h)                                 # (B, attn_out_channel, N*up_factor, k)

        # softmax over neighbors
        attention = self.scale(h)

        value_group = value_group + pos_emb + up_rel           # (B, dim, N, k)
        value_up    = self.upsample1(value_group)              # (B, dim, N*up_factor, k)

        agg = einsum('b c i j, b c i j -> b c i', attention, value_up)  # (B, dim, N*up_factor)

        # Spiking conv_end over time: expand agg over T and spike it
        agg_seq = agg.unsqueeze(0).repeat(self.T, 1, 1, 1)     # (T, B, dim, N*up_factor)
        out_seq = self.conv_end_spike(agg_seq)                 # (T, B, out_channel, N*up_factor)
        y = out_seq[-1]                                        # final readout (B, out_channel, N*up_factor)

        # Residual upsample path (non-spiking)
        identity = self.residual_layer(identity)               # (B, out_channel, N)
        identity = self.upsample2(identity)                    # (B, out_channel, N*up_factor)

        # Reset spiking states for next batch
...
class UpLayer(nn.Module):
    """
    Upsample Layer with upsample transformer
    """
    def __init__(self, dim, seed_dim, up_factor=2, i=0, radius=1, n_knn=20, interpolate='three', attn_channel=True):
        ...
        #spikingUpTransformer
        self.uptrans1 = SpikingUpTransformer(dim, dim, dim=64, n_knn=self.n_knn, use_upfeat=True, up_factor=None, T=4)
        self.uptrans2 = SpikingUpTransformer(dim, dim, dim=64, n_knn=self.n_knn,use_upfeat=True, attn_channel=attn_channel,up_factor=self.up_factor, T=4)
...
class SpikingUpsampleUpPoinTr(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
